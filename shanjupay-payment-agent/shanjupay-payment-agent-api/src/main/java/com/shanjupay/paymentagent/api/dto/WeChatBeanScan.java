...
public class WeChatBeanScan implements Serializable {
    ...
    // 微信付款码支付的必传参数。
    private String appid;// 公众号 此处不用传，设置 sdk 里有设置。
    private String mchId;// 商户号。
    private String nonceStr;// 随机字符串。
    private String sign;// 签名 此处不用设置 sdk 里有设置。
    private String body;// 商品描述。
    private String outTradeNo;// 商户订单号。
    private Integer totalFee;// 标价金额 订单总金额，单位为分，只能为整数，详见支付金额。
    private String spbillCreateIp;// 终端 IP 8.8.8.8。支持 IPV4 和 IPV6 两种格式的 IP 地址。调用微信支付 API 的机器 IP。
    private String authCode;// 授权码。120061098828009406	扫码支付授权码，设备读取用户微信中的条码或者二维码信息（注：用户付款码条形码规则：18位纯数字，以 10、11、12、13、14、15 开头）。

    // 微信付款码支付的非传参数。
    private String signType;// 签名类型。此处不用设置 sdk 里有设置。目前支持 HMAC-SHA256 和 MD5，沙箱环境为MD5。正式环境是 HMAC-SHA256。如果是此类型需设置交易过程生成签名的密钥。
    ...
    private String time_start;// 交易起始时间 String(14)	20091225091010	订单生成时间，格式为 yyyyMMddHHmmss，如2009年12月25日9点10分10秒表示为20091225091010。其他详见时间规则。
    private String time_expire;// 交易结束时间 String(14)	20091227091010	订单失效时间，格式为 yyyyMMddHHmmss，如2009年12月27日9点10分10秒表示为20091227091010。注意：最短失效时间间隔需大于1分钟。
