...
public class AlipayBCScanBean implements Serializable {
    ...
    // 需保证商户系统端不能重复，建议通过数据库 sequence 生成。
    private String outTradeNo;

    // (必选) 支付场景，条码支付场景为 bar_code。
    private String scene;

    // (必选) 付款条码，用户支付宝钱包手机 app 点击“付款”产生的付款条码。
    private String authCode;

    // (可选) 销售产品码。该字段不要传，否则会报 ACQ.ACCESS_FORBIDDEN 无权限使用接口 未签约条码支付或者合同已到期。
    private String productCode;

    // (可选) 订单标题，粗略描述用户的支付目的。如“喜士多（浦东店）消费”。
    private String subject;

    // (可选) 买家的支付宝用户id，如果为空，会从传入了码值信息中获取买家 ID 2088202954065786/
    private String buyerId;

    // (可选) 卖家支付宝账号ID，用于支持一个签约账号下支持打款到不同的收款账号，(打款到sellerId对应的支付宝账号)。
    // 如果该字段为空，则默认为与支付宝签约的商户的 PID，也就是 appid 对应的 PID。
