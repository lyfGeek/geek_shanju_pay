...
public class PayChannelAgentServiceImpl implements PayChannelAgentService {
    ...
    public PaymentResponseDTO createPayOrderByAliWAP(AliConfigParam aliConfigParam, AlipayBean alipayBean) throws BusinessException {
        ...
        String appId = aliConfigParam.getAppId();// 支付宝应用 id。
        String rsaPrivateKey = aliConfigParam.getRsaPrivateKey();// 应用私钥。
        String format = aliConfigParam.getFormat();// json 格式。
        String charest = aliConfigParam.getCharset();// 编码。
        String alipayPublicKey = aliConfigParam.getAlipayPublicKey();// 支付宝公钥。
        String signtype = aliConfigParam.getSignType();// 签名算法。
        String returnUrl = aliConfigParam.getReturnUrl();// 支付成功跳转的 url。
        String notifyUrl = aliConfigParam.getNotifyUrl();// 支付结果异步通知的 url。

        // 构造 sdk 的客户端对象。
        AlipayClient alipayClient = new DefaultAlipayClient(url, appId, rsaPrivateKey, format, charest, alipayPublicKey, signtype); // 获得初始化的 AlipayClient。
        ...
        model.setProductCode("QUICK_WAP_PAY");// 产品代码，固定 QUICK_WAP_PAY。
    ...
    public Map<String, String> createPayOrderByWeChatJSAPI(WXConfigParam wxConfigParam, WeChatBean weChatBean) {
        ...
        try {
            ...
            requestParam.put("spbill_create_ip", weChatBean.getSpbillCreateIp());// 客户端 ip。
