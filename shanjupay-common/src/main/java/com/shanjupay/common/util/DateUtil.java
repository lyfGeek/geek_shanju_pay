...
public class DateUtil {
    ...
    /**
     * 从给定的 date，加上 hour 小时
     * 求指定 date 时间后 hour 小时的时间。
     *
     * @param date 指定的时间。
     * @param hour 多少小时后。
     * @return
...
//            day_month = cad.getActualMaximum(Calendar.DAY_OF_MONTH);// 获取当月天数。
     ...
     * @return
     */
    public static String getSpecifiedDayBefore(String specifiedDay, String formatStr) {// 可以用 new Date().toLocalString()传递参数。
    ...
    public static final String getStartDayWithMonth(String month) throws ParseException {
        ...
        calendar.add(Calendar.DATE, 0);// 因为格式化时默认了 DATE 为本月第一天所以此处为 0。
    ...
    public static final String getEndDayWithMonth(String month) throws ParseException {
        ...
        Date date = mf.parse(month);
        calendar.setTime(date);
        calendar.roll(Calendar.DATE, -1);// api 解释 roll()：向指定日历字段添加指定（有符号的）时间量，不更改更大的字段。
        return sdf.format(calendar.getTime());
    }
