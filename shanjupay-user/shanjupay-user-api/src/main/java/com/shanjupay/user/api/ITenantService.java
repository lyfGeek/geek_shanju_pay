...
public interface ITenantService {
    ...
    /**
     * 切换租户套餐。
     * 租户切换套餐操作会清除原租户内的所有账号-角色关联数据以及原套餐产生的角色权限数据，并将限流规则写入 sentinel。
     *
...
//    void bindTenantAccount(Long tenantId, Long accountId, Boolean isAdmin) throws BusinessException;
    ...
    /**
     * 用户登录，被uaa服务调用并生成令牌。
     * 1.调用 authentication(AuthenticationInfo authenticationInfo); 认证接口通过认证。
     * 2.调用 queryAccountInTenant(String username); 获取该用户所属租户列表。
     * 3.授权，调用 AuthorizationService.authorize(String username, Long[] tenantIds); 获取该用户在多个租户下的权限。
     * 4.获取资源，调用 loadResources(Map<Long, AuthorizationInfoDTO> tenantAuthorizationInfMap); 获取该用户在多个租户下的资源。
     ...
     * @throws BusinessException
     */
    TenantDTO getTenantByAccount(Long accountId, Boolean isAdmin) throws BusinessException;
