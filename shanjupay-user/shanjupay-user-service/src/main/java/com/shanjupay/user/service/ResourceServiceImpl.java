...
import com.shanjupay.user.convert.IResourceApplicationConvert;
...
import com.shanjupay.user.mapper.IResourceApplicationMapper;
import com.shanjupay.user.mapper.IResourceButtonMapper;
...
public class ResourceServiceImpl implements IResourceService {
    ...
    @Autowired
    private IResourceApplicationMapper applicationMapper;
    @Autowired
    private IResourceMenuMapper menuMapper;
    @Autowired
    private IResourceButtonMapper buttonMapper;
    //@Reference
    ...
    public void createApplication(ApplicationDTO application) {
        ResourceApplication entity = IResourceApplicationConvert.INSTANCE.dto2entity(application);
    ...
    public ApplicationDTO queryApplication(String applicationCode) {
        ...
        ApplicationDTO applicationDTO = IResourceApplicationConvert.INSTANCE.entity2dto(application);
    ...
    // 入参：多个租户下的角色权限信息，key 为租户 id,value 为租户下的角色权限信息.
    ...
    public Map<Long, List<ResourceDTO>> loadResources(Map<Long, AuthorizationInfoDTO> tenantAuthorizationInfoMap) {
        ...
        for (Map.Entry<Long, AuthorizationInfoDTO> entry : tenantAuthorizationInfoMap.entrySet()) {
            ...
            AuthorizationInfoDTO authInfo = entry.getValue();// 租户下的角色权限 map。结构：rolecode1 [pcode1, pcode2]
            Map<String, List<String>> rolePrivilegeMap = authInfo.getRolePrivilegeMap();
            // 遍历角色权限的 map。
            Set<String> privilegeCodeSet = new HashSet<>();// 权限编码集合。
            for (Map.Entry<String, List<String>> entry2 : rolePrivilegeMap.entrySet()) {// key 为角色编码，value 为权限编码。
                // roleCode = entry2.getKey();
                // 租户某个角色下的权限编码 list，一个角色可能对应多个权限。
...
//     * 获取多个租户下权限所对应的的资源信息，并按应用拆分。
//     *
//     * @param tenantAuthorizationInfoMap 多个租户下的角色权限信息，key 为租户 id，value 为租户下的角色权限信息。
//     * @return 多个租户下的资源信息，key 为租户 id，value 为租户下多个应用的资源信息。
//     * eg. {租户A: [{应用1资源信息}, {应用2资源信息}], 租户B: [{}, {}, ...]}。
//     */
//    @Override
//    public Map<Long, List<ResourceDTO>> loadResources(Map<Long, AuthorizationInfoDTO> tenantAuthorizationInfoMap) {
//        if (tenantAuthorizationInfoMap.isEmpty()) {
...
//        }
//        /**
//         * - 传入多个租户（key）-角色权限，最终要租户-按应用划分-资源信息。
//         * - 根据权限获取资源 loadResources(List<String> privilegeCodes, String applicationCode);
//         */
////        /*
//        Map<Long, List<ResourceDTO>> resultResource = new HashMap<>();
//        // Map<String, List<ResourceDTO>> tmpMap = new HashMap<>();
//        // 遍历 map 获取多个租户的角色权限信息。
//        for (Map.Entry<Long, AuthorizationInfoDTO> entry : tenantAuthorizationInfoMap.entrySet()) {
//            AuthorizationInfoDTO dto = entry.getValue();// 结构：rolecode1 [pcode1, pcode2]
//            Map<String, List<String>> rolePrivilegeMap = dto.getRolePrivilegeMap();// 角色-关系数据 Map<String,List<String>>。
//
//            Set<String> privilegeCodeSet = new HashSet<>();// 权限编码集合。
//            // 遍历角色权限的 map。Map<String,List<String>> 结构：rolecode1 [pcode1,pcode2]。
//            for (Map.Entry<String, List<String>> entry2 : rolePrivilegeMap.entrySet()) {
//                List<String> pcodelist = entry2.getValue();// 租户某个角色下的权限 list，一个角色可能对应多个权限。
//                // 根据权限获取资源。
//                privilegeCodeSet.addAll(pcodelist);
//            }
//
...
//                    String resource = resourceLoader.load(privilegeCodes, applicationCode);
...
//                obj.add(m);
//                resourceDTO.getAppRes().put("menu", obj);// 不同类型的资源加载。待完善：注入一个接口，获取资源类型的 list。
//                r.add(resourceDTO);
//            }
