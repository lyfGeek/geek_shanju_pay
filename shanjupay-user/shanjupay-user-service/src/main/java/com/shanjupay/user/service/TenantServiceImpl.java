...
import com.shanjupay.user.api.IAuthorizationService;
import com.shanjupay.user.api.IResourceService;
import com.shanjupay.user.api.ITenantService;
...
public class TenantServiceImpl implements ITenantService {
    ...
    @Autowired
    private IBundleMapper bundleMapper;
    @Autowired
    private IAccountMapper accountMapper;
    @Autowired
    private ITenantAccountMapper tenantAccountMapper;
    @Autowired
    private IAccountRoleMapper accountRoleMapper;
    @Autowired
    private IAuthorizationRoleMapper roleMapper;
    @Autowired
    private IAuthorizationRolePrivilegeMapper rolePrivilegeMapper;
    @Autowired
    private IAuthorizationService authorizationService;
    @Autowired
    private IResourceService resourceService;
    @Autowired
    private IResourceApplicationMapper resourceApplicationMapper;
    ...
    /**
     * 创建租户。
     * 新增租户、新增租户管理员，同时初始化套餐（预留：将限流规则写入 sentinel）。
     * - 若管理员用户名已存在，禁止创建。
     * - 手机号已存在，禁止创建。
     * - 创建根租户对应账号时，需要手机号，账号的用户名密码和盐需要随机生成。
     *
    ...
    public TenantDTO createTenantAndAccount(CreateTenantRequestDTO createTenantRequest) {
        ...
        if (isExistAccountByMobile(mobile)) {// 为 true 表示手机号已存在。
// throw new BusinessException(CommonErrorCode.E_200203);
            ...
            if (tenant == null) {
                ...
                return ITenantConvert.INSTANCE.entity2dto(ten);
            } else {
                return tenant;
            }
            ...
            return ITenantConvert.INSTANCE.entity2dto(tenant);
        }
    }
    ...
    public TenantDTO getTenant(Long id) {
        ...
        return ITenantConvert.INSTANCE.entity2dto(entity);
    }
    ...
    public List<BundleDTO> queryBundleByTenantType(String tenantType) {
        ...
        List<BundleDTO> dtoList = IBundleConvert.INSTANCE.entityList2dto(bundles);
        return dtoList;
    }
    ...
    public BundleDTO getBundle(String bundleCode) {
        ...
        return IBundleConvert.INSTANCE.entity2dto(entity);
    }
    ...
    public List<BundleDTO> queryAllBundle() {
        ...
        return IBundleConvert.INSTANCE.entityList2dto(bundles);
    }

    /**
     * 切换租户套餐。
     * 租户切换套餐操作会清除原租户内的所有账号-角色关联数据，原套餐产生的角色权限数据，并将限流规则写入 sentinel。
     *
    ...
    public void initBundle(Long tenantId, String bundleCode) {
        ...
        if (bundle != null && StringUtils.isNotBlank(bundle.getAbility())) {
            ...
            /* ability 格式如下。
                    [
                    {"name":"商户管理员", "code":"r_001", "privilegeCodes": [
            "p_m_001", "p_m_002", "p_m_003", "p_m_004", "p_m_005"]},
            {
                "name":"商户门店收银员", "code":"r_002", "privilegeCodes": ["p_m_014"]}
        }*/
            // 处理角色权限 json（角色和权限的 code），为租户绑定角色和权限。
            List<RoleDTO> roleDTOList = JSONObject.parseArray(ability, RoleDTO.class);
            List<AuthorizationRole> roles = IAuthorizationRoleConvert.INSTANCE.dtoList2entity(roleDTOList);
            List<String> rCodes = new ArrayList<>();// 构建该套餐中的角色集合。
            roles.forEach(role -> rCodes.add(role.getCode()));

            // 在指定租户下新增角色。操作 authorization_role 表。
            roleMapper.createRoles(tenantId, roles);

            // 为租户绑定角色。操作 account_role 表。
            ...
            // 为租户下的角色绑定权限。操作 authorization_role_privilege 表。
    ...
    public void createBundle(BundleDTO bundleDTO) {
        Bundle bundle = IBundleConvert.INSTANCE.dto2entity(bundleDTO);
    ...
    /**
     * 将已存在账号，绑定到某租户。
     * 界面上先通过手机号查询到账号信息，再通过账号信息的 username，调用此接口，加入租户。
     *
     * @param tenantId 租户 id。
     * @param username 用户名。
     */
    ...
    public AccountDTO createAccount(CreateAccountRequestDTO createAccountRequest) {
        ...
        return IAccountConvert.INSTANCE.entity2dto(account);
    }
    ...
    /**
     * 用户登录，被 uaa 服务调用并生成令牌。
     * - 调用 authentication(AuthenticationInfo authenticationInfo);认证接口通过认证。
     * - 调用 queryAccountInTenant(String username);获取该用户所属租户列表。
     * - 授权，调用AuthorizationService.authorize(String username, Long[] tenantIds);获取该用户在多个租户下的权限。
     * - 获取资源,调用loadResources(Map<Long, AuthorizationInfoDTO> tenantAuthorizationInfMap);获取该用户在多个租户下的资源。
     *
     * @param loginRequest 登录请求。
     * @return 登录响应，包括用户的租户、角色、权限资源等信息。
     * <p>
     * 备忘录。
     * - 由于 token 设计的可以跨多个租户，当前登入租户，需由应用方(可默认选第一个租户)掌控并传递，设计在有状态接口的参数中，不然无法知道用户当前的登入租户。
     * - 在 uaa 生成 token 前，需要根据 client_id 获取当前接入应用所属租户 ID（IResourceService.queryApplication(String applicationCode);），并放入 token，此 tenantId 作为限流的 limitApp，也是套餐拥有者标识，用于套餐限制。
     * - token 不能跨不同租户开发的应用。
     */
    @Override
...
//            loginInfo.setResources(resources);
    ...
    public ApplicationDTO getApplicationDTOByClientId(String clientId) {
        ...
        return IResourceApplicationConvert.INSTANCE.entity2dto(resourceApplication);
    }
