...
public interface ITransactionService {
     ...
     * 生成门店二维码的 url。
     *
     * @param qrCodeDto 传入 merchantId，appId，storeId，channel，subject，body。
     * @return 支付入口（url），要携带参数（将传入的参数转成 json，用 base64 编码）。
     ...
     * @param state             订单状态。交易状态 / 支付状态。0 ~ 订单生成。1 ~ 支付中（目前未使用）。2 ~ 支付成功。4 ~ 关闭。5 ~ 失败。
     */
     ...
     *
     * @param code  授权码。
     * @param appId 闪聚平台的应用 id，为了获取该应用的微信支付渠道参数。
     * @return
     */
